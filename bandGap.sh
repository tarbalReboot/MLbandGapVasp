#!/bin/sh
           awk -v fermi=$(gawk '/E-fermi/ {print $3}' OUTCAR)                      # rip fermi energy from OUTCAR
           awk -v nbands=$(gawk '/NBANDS/ {print $15}' OUTCAR)                      # rip nbands from OUTCAR
	   ...
	   nbands2=$(awk '{print nbands+2}')                                    # set line to skip    
		...
		awk -v l=$i
		j=$(awk '{print l - fermi}')                                        #    fermi
           echo "$j" >> band_gaptempfile3                                   #    energy
           done                                                             #    from EIGENVALS
           gawk '/<r>/ min==""|| $1 > 0 {print $1}' band_gaptempfile3 > band_gaptempfile4    # rip + eigenvals
           awk -v top=$(gawk 'min==""||min>$1{min=$1}END{print min}' band_gaptempfile4)             # cond band min
           gawk '/<r>/ max==""|| $1 < 0 {print $1}' band_gaptempfile3 > band_gaptempfile5    # rip - eigenvals
           awk -v bottom=$(gawk 'max==""||max<$1{max=$1}END{print max}' band_gaptempfile5)          # val band max
